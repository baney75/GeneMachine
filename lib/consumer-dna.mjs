...
function detectOrientation(comments) {
  const joined = comments.join(" ");
  const forward = /(?:forward|positive|plus|\+)\s*(?:strand|orientation)|(?:strand|orientation)\s*(?:is|:)?\s*(?:forward|positive|plus|\+)/i.test(joined);
  const reverse = /(?:reverse|negative|minus|-)\s*(?:strand|orientation)|(?:strand|orientation)\s*(?:is|:)?\s*(?:reverse|negative|minus|-)/i.test(joined);
  if (forward && !reverse) {
