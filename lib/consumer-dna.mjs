...
function detectOrientation(comments) {
  const joined = comments.join(" ");
  const orientationNoun = String.raw`(?:strand|orientation)`;
  const forwardLabel = String.raw`(?:(?:forward|positive|plus)(?:\s*\(\s*\+\s*\))?|\(\s*\+\s*\))`;
  const reverseLabel = String.raw`(?:(?:reverse|negative|minus)(?:\s*\(\s*-\s*\))?|\(\s*-\s*\))`;
  const declared = (label) => new RegExp(
    String.raw`(?:${label})[\s-]*${orientationNoun}|${orientationNoun}\s*(?:is|:|=)?\s*(?:the\s*)?(?:${label})`,
    "i",
  ).test(joined);
  const forward = declared(forwardLabel);
  const reverse = declared(reverseLabel);
  if (forward && !reverse) {
