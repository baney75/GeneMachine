...
function detectOrientation(comments, provider) {
  ...
  if (hasInvalidDirectionEvidence) {
    ...
    const nounFirstSymbol = (symbol) => {
      const escapedSymbol = symbol === "+" ? String.raw`\+` : String.raw`\-`;
      return new RegExp(
        String.raw`(?:strand|orientation)\s*(?:(?:is|reported\s+(?:as|on))\s*|:|=)\s*(?:the\s+)?(?:\(\s*${escapedSymbol}\s*\)|${escapedSymbol})(?=$|[\s.;,])`,
        "i",
      );
    };
    const hasForwardSymbol = (
      /\(\s*\+\s*\)|(?:^|[\s:;,/])\+\s*(?:strand|orientation)/i.test(combinedHeader)
      || nounFirstSymbol("+").test(combinedHeader)
    );
    const hasReverseSymbol = (
      /\(\s*-\s*\)|(?:^|[\s:;,/])-\s*(?:\/\s*\+\s*)?(?:strand|orientation)/i.test(combinedHeader)
      || nounFirstSymbol("-").test(combinedHeader)
    );
