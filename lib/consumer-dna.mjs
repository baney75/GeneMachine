...
function detectOrientation(comments) {
  ...
  const invalidDirectionLanguage = new RegExp(
    String.raw`\b(?:not|never|neither|no|without|unknown|ambiguous|undetermined|unspecified|uncertain|unconfirmed|possibly?|may|might|assumed|presumed|inferred)\b|\b(?:non|un)[-_]?${directionWords}\b|\b${directionWords}[-_]${directionWords}\b|\b${forwardWords}\b[^.;]{0,40}\b${reverseWords}\b|\b${reverseWords}\b[^.;]{0,40}\b${forwardWords}\b`,
    "i",
  );
  const declarationLines = comments
    .map((comment) => comment.replace(/^#+\s*/, "").trim())
    .filter((comment) => comment && !invalidDirectionLanguage.test(comment));
