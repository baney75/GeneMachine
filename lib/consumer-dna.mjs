...
function detectOrientation(comments) {
  ...
  const qualifier = String.raw`(?:not|never|neither|no|without|unknown|ambiguous|undetermined|unspecified|uncertain|unconfirmed|possibly?|perhaps|maybe|may|might|could|cannot|assumed|presumed|inferred)`;
  const qualifierPattern = new RegExp(String.raw`\b${qualifier}\b`, "i");
  const orientationNounPattern = /\b(?:strand|orientation)\b/i;
  const prefixedOrCompoundPattern = new RegExp(
    String.raw`\b(?:non|un)[-_]?${directionWords}\b|\b${directionWords}[-_]${directionWords}\b|\b${forwardWords}\b[\s\S]*\b${reverseWords}\b|\b${reverseWords}\b[\s\S]*\b${forwardWords}\b`,
    "i",
  );
  const standaloneQualifierPattern = new RegExp(
    String.raw`^${qualifier}(?:\s+(?:confirmed|known|declared|specified))?[\s.!;:,-]*$`,
    "i",
  );
  const declarationLines = comments
    .map((comment) => comment.replace(/^#+\s*/, "").trim())
    .filter(Boolean);
  const isDeclared = (line, words, symbol) => {
    ...
    const beforeNoun = new RegExp(
      String.raw`(?:^|[\s:;,])${label}[\s-]*(?:strand|orientation)\b`,
      "i",
    );
    const afterNoun = new RegExp(
      String.raw`\b(?:strand|orientation)\b\s*(?:(?:is|reported\s+(?:as|on)|:|=)\s*)?(?:the\s*)?${label}(?:\b|$)`,
      "i",
    );
    return beforeNoun.test(line) || afterNoun.test(line);
  };
  const lineEvidence = declarationLines.map((line) => ({
    line,
    forward: isDeclared(line, String.raw`forward|positive|plus`, "+"),
    reverse: isDeclared(line, String.raw`reverse|negative|minus`, "-"),
  }));
  const anyOrientationLanguage = new RegExp(String.raw`\b(?:strand|orientation|${directionWords})\b`, "i");
  const hasInvalidDirectionEvidence = declarationLines.some((line) => (
    (qualifierPattern.test(line) && orientationNounPattern.test(line))
    || standaloneQualifierPattern.test(line)
    || prefixedOrCompoundPattern.test(line)
    || (/\b(?:strand|orientation)\b/i.test(line) && line.includes("+") && line.includes("-"))
  )) || lineEvidence.some(({ line, forward, reverse }) => (
  ...
  if (hasInvalidDirectionEvidence) {
    ...
    const hasConflictingDirections = (
      ...
      || (combinedHeader.includes("+") && combinedHeader.includes("-") && /\b(?:strand|orientation)\b/i.test(combinedHeader))
    );
    return {
      value: null,
...
export function parseConsumerDna(text, fileName = "consumer-dna.txt", options = {}) {
  ...
  if (text.length > effectiveLimit) {
    throw new Error(`This file is too large to inspect safely in the browser (${text.length.toLocaleString()} characters; limit ${effectiveLimit.toLocaleString()}).`);
  }
  if (/^##fileformat=VCF|^#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT/im.test(text.slice(0, 100_000))) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (raw.startsWith("#")) comments.push(raw.slice(0, 500));
    if (!candidateDelimiter) continue;
    const possibleColumns = mapColumns(splitLine(candidate, candidateDelimiter));
    if (possibleColumns) {
      const normalized = splitLine(candidate, candidateDelimiter).map(normalizedHeader);
  ...
  const orientation = detectOrientation(comments);
