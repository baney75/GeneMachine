...
function detectOrientation(comments) {
  ...
  const orientationTerm = String.raw`(?:strand|orientation|${directionWords})`;
  const qualifier = String.raw`(?:not|never|neither|no|without|unknown|ambiguous|undetermined|unspecified|uncertain|unconfirmed|possibly?|maybe|may|might|could|cannot|assumed|presumed|inferred)`;
  const invalidDirectionLanguage = new RegExp(
    String.raw`\b${qualifier}\b[^.;]{0,60}\b${orientationTerm}\b|\b${orientationTerm}\b[^.;]{0,60}\b${qualifier}\b|\b(?:non|un)[-_]?${directionWords}\b|\b${directionWords}[-_]${directionWords}\b|\b${forwardWords}\b[^.;]{0,40}\b${reverseWords}\b|\b${reverseWords}\b[^.;]{0,40}\b${forwardWords}\b`,
    "i",
  );
  ...
  const hasInvalidDirectionEvidence = declarationLines.some((line) => (
    invalidDirectionLanguage.test(line)
  ...
  const declared = (words, symbol) => {
    ...
    return declarationLines.some((line) => beforeNoun.test(line) || afterNoun.test(line));
  };
  const forward = declared(String.raw`forward|positive|plus`, "+");
  const reverse = declared(String.raw`reverse|negative|minus`, "-");
  if (forward && !reverse) {
